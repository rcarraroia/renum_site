"""
Sprint 06 - Wizard API Integration Tests (With Authentication)
Tests all wizard endpoints with proper authentication
"""
...
# Test credentials - UPDATE THESE WITH REAL TEST USER
TEST_EMAIL = "admin@renum.com"  # Change to your test user
...
if not auth_token:
    ...
    print("Or create a test user in Supabase")
    sys.exit(1)
...
if wizard_id:
    try:
        ...
        if response.status_code == 201:
            ...
            log_test("Start Sandbox", False, f"Status code: {response.status_code}, Response: {response.text}")
    except Exception as e:
...
if wizard_id:
    try:
        ...
        if response.status_code == 200:
            ...
            log_test("Publish Agent", False, f"Status code: {response.status_code}, Response: {response.text}")
    except Exception as e:
...
if wizard_id:
    try:
        response = requests.delete(f"{BASE_URL}/agents/wizard/{wizard_id}", headers=headers)
        if response.status_code == 204:
